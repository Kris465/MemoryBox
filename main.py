...
async def cmd_start(message: types.Message):
    await message.answer("Привет! Я бот для тестирования студентов. Используй команду /topics, чтобы увидеть доступные тесты.")
...
@dp.message_handler(lambda message: message.text in load_tests().keys())
async def choose_topic(message: types.Message, state: FSMContext):
    ...
    for question in questions:
        ...
        answer = await bot.wait_for(types.Message)  # Ожидаем ответа пользователя
...
async def cmd_rating(message: types.Message):
    if os.path.exists('users.json'):
        ...
        rating_list = "\n".join([f"ID: {user_id}, Тема: {data['topic']}, Баллы: {data['score']}" for user_id, data in users.items()])
