...
class EthicsAssistant:
    ...
    def find_relevant_rule(self, user_question: str) -> str:
        ...
        for entry in self.knowledge_base:
            ...
            if topic == 'знакомство':
                keywords = ['знакомств', 'предста', 'руку',
                            'подать', 'познаком', 'представ']
            elif topic == 'переписка':
                keywords = ['переписк', 'письм', 'email', 'сообщен', 'капс',
                            'caps', 'мессенджер', 'whatsapp', 'телеграм']
            elif topic == 'звонок':
                keywords = ['звонок', 'позвонить', 'телефон', 'звонить',
                            'поздрав', 'созвонить']
            elif topic == 'соцсети':
                keywords = ['соцсет', 'социальн', 'instagram', 'инстаграм',
                            'facebook', 'фейсбук', 'вконтакте', 'vk',
                            'twitter', 'твиттер', 'фото', 'photograph',
                            'публикац']
            elif topic == 'разговор':
    ...
    def generate_response(self, user_question: str) -> str:
        ...
        if "TinyLlama" in self.model_name:
            prompt = f"""<|system|>
            Ты ассистент по этикету.
            Отвечай ТОЛЬКО на основе предоставленного правила.
            ...
            prompt = f"""Ты консультант по этикету.
            Ответь на вопрос строго на основе предоставленного правила.
