...
class Stepper(scrapy.Spider):
    ...
    async def parse_chapter_links(self, response):
        ...
        try:
            # More flexible selector that waits for chapter elements
            await page.wait_for_selector(
                'section a[href*="/chapters/"]',  # CSS selector looking for chapter links
                timeout=30000
            )

            # Get all chapter links using a more reliable selector
