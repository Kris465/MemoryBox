...
chapters = {}
# https://inoveltranslation.com/novels/eaedecb0-0281-4667-a79e-e97a7ac9ef2a

def get_novel(url):
    chapter = 1
    soup = get_webpage(url)
    chapters_list = soup.find("section", class_='py-32')
    ...
    links.reverse()
    for link in links:
        chapter_page = get_webpage('https://inoveltranslation.com' + link)
        text = chapter_page.find("main", class_="styles_content__JHK8G lg:px-24 md:px-16 px-8").text
        result = []
        for char in text:
            if char == '.':
                result.append('.\n')
                ...
                result.append(char)
        text = ''.join(result)
        chapters.update({str(chapter): 
            'https://inoveltranslation.com' + link + text})
        chapter += 1
