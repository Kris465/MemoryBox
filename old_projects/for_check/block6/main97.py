def find_place(sorted_points, target_points):
    ...
    while left <= right:
        ...
        if sorted_points[mid] == target_points:
            return mid + 1  # +1, так как индексация начинается с 0
        elif sorted_points[mid] < target_points:
            right = mid - 1
        else:
            left = mid + 1
    
    return -1  # Если значение не найдено (хотя по условию задачи оно всегда есть)

# Пример использования
sorted_points = [56, 54, 53, 50, 49, 48, 47, 45, 44, 43, 42, 40, 39, 38, 37, 36, 35, 34, 33, 32]
target_points = 47
